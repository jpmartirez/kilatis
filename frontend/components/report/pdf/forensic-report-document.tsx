		...
		>
				...
				<View style={{ flex: 1, display: "flex", flexDirection: "column" }}>
					...
					<View>
						...
						<Text style={styles.headerSubtitle}>
							Automated tri-stream tamper detection · Case {caseNumber} ·
							Prepared for PNP Anti-Cybercrime Group
						</Text>
					</View>
						...
						<View style={styles.innerWhiteBox}>
							...
							<View style={[styles.grid2Col, { marginTop: 6 }]}>
								<View style={styles.col}>
									...
									<Text
										style={[styles.valueText, { textTransform: "uppercase" }]}
									>
										{caseTitle}
									</Text>
								</View>
								<View style={styles.col}>
									<Text style={styles.labelSmall}>INVESTIGATOR / EXAMINER</Text>
									<Text
										style={[styles.valueText, { textTransform: "uppercase" }]}
									>
										{investigatorName}
					...
					<View style={styles.sectionCard}>
						...
						<View style={styles.innerWhiteBox}>
							<Text style={[styles.paragraphText, { marginBottom: 8 }]}>
								Performance varies by tampering type and image quality; heavily
								compressed or low-resolution images reduce reliability,
								particularly for the frequency-based indicator.
							</Text>
							...
							<View style={{ marginTop: 2 }}>
								<View
									style={[
										styles.grid2Col,
										{ borderBottom: "0.5px solid #f1f5f9", paddingBottom: 3 },
									]}
								>
									...
									>
										Tri-stream analysis completed
									</Text>
									<Text style={{ fontSize: 8, color: "#475569" }}>
										KILATIS-TRISTREAM V1.0
									</Text>
								</View>
								<View
									style={[
										styles.grid2Col,
										{ borderBottom: "0.5px solid #f1f5f9", paddingVertical: 3 },
									]}
								>
									<Text style={{ fontSize: 8, color: "#475569" }}>
										{timestampShort}
			...
			{items.map((item: StoredResultItem, idx: number) => {
				...
				const noisePct = Math.round(
					(result.streams?.noise_score ?? result.scores?.p_splice ?? 0.25) *
						100,
				);
				const freqPct = Math.round(
					(result.streams?.frequency_score ?? result.scores?.p_ai ?? 0.34) *
						100,
				);

				const pSplice =
					result.class_probabilities?.traditional_spliced ??
					result.scores?.p_splice ??
					(isSpliced ? 0.93 : 0.08);
				const pAiDeepfake =
					result.class_probabilities?.ai_deepfake ??
					result.scores?.p_ai ??
					(isAi ? 0.92 : 0.12);
				const pAuth =
					result.class_probabilities?.authentic ??
					(isAuthentic
						? 0.93
					...
					<Page key={idx} size="A4" style={styles.page}>
						...
						>
									...
									<View style={{ width: "70%" }}>
										{isSpliced ? (
											<>
												...
												>
													All three detection methods agree that this image has
													been altered. The evidence is consistent with{" "}
													<Text style={{ fontFamily: "Helvetica-Bold" }}>
														image splicing
													</Text>{" "}
													— content copied from a separate source photograph and
												...
												>
													The system located the affected area in the
													upper-right portion of the frame, covering roughly 8%
													of the image. This finding is driven primarily by
													visual-structure and sensor-noise evidence;
													frequency-domain evidence was comparatively weak.
												</Text>
											</>
										) : isAi ? (
											<>
											...
											<>
												<Text
													style={[styles.paragraphText, { marginBottom: 4 }]}
												>
													The image exhibits consistent physical sensor noise
													and natural optical characteristics with{" "}
													<Text style={{ fontFamily: "Helvetica-Bold" }}>
														no evidence of splicing or AI tampering
													</Text>
													.
												</Text>
												<Text
												...
												>
													PRNU sensor pattern noise distribution and compression
													boundaries match native camera hardware capture
													properties.
												</Text>
											</>
										)}
							...
							<View style={styles.sectionCard}>
								...
								<Text
									style={{ fontSize: 8, color: "#0f172a", marginBottom: 6 }}
								>
								...
								<View style={styles.innerWhiteBox}>
									<View style={styles.streamRow}>
										...
										<Text style={styles.streamCol2}>
											The edges of the highlighted region show a sharpness and
											shading pattern that does not match the surrounding area,
											consistent with content pasted in from a different
											photograph.
										</Text>
									</View>
									...
									<View style={styles.streamRow}>
										...
										<Text style={styles.streamCol2}>
											The region&apos;s sensor noise pattern breaks from the
											rest of the image, suggesting it originated from a
											different source image or capture device.
										</Text>
									</View>
									...
									<View style={styles.streamRow}>
										...
										<Text style={styles.streamCol2}>
											This indicator showed comparatively low deviation — the
											file&apos;s compression structure does not by itself
											strongly suggest AI generation or resave manipulation.
										</Text>
									</View>
								</View>
							</View>
										...
										>
											<View style={{ alignItems: "center" }}>
												<View
													style={{
														width: 10,
														height: Math.max(8, Math.round(pAuth * 36)),
											...
											<View style={{ alignItems: "center" }}>
												<View
													style={{
														width: 10,
														height: Math.max(8, Math.round(pSplice * 36)),
				...
				>
					...
					<View style={styles.sectionCard}>
						...
						<View style={styles.innerWhiteBox}>
							<View style={{ marginBottom: 8 }}>
								<Text
									style={{ fontSize: 7.5, color: "#1e293b", marginBottom: 3 }}
								>
									( ) I certify that I have reviewed the automated findings
									above against the source evidence file.{" "}
									<Text
								...
								<Text style={{ fontSize: 7.5, color: "#1e293b" }}>
									( ) I acknowledge that this report accurately reflects the
									KILATIS system output at the time of analysis.{" "}
									<Text
										style={{ color: "#ef4444", fontFamily: "Helvetica-Bold" }}
									>
										*Required
							...
							<View style={styles.grid2Col}>
								<View style={[styles.col, { marginRight: 8 }]}>
									...
									<View style={styles.blankInputBox}>
										<Text style={styles.inputText}>{investigatorName}</Text>
									</View>
								</View>
					...
					<View style={styles.warningBanner}>
						<Text style={styles.warningBannerText}>
							AUTOMATED PRELIMINARY ANALYSIS — NOT A SUBSTITUTE FOR EXPERT
							REVIEW
						</Text>
					</View>
					...
					<View style={styles.disclaimerBox}>
						<Text style={styles.disclaimerText}>
							This report was produced by an automated preliminary screening
							tool developed for research and investigative-assistance purposes.
							It is not a substitute for manual forensic examination by a
							qualified digital forensics expert, and should not be relied upon
							as sole evidentiary basis in any legal proceeding without
